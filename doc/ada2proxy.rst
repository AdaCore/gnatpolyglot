...
The Ada2Proxy tool is invoked as a subcommand of the GNATpolyglot program.
It must be provided with at least a project file and an output path:

.. code:: sh

   $> gnatpolyglot ada2proxy -P<project-file> -o<output-path> {<switch>}
...
* ``-P=<project-file>``
   ...
   Input GPR project file.

* ``-o|--output=<outputPath>``

   Output path for the proxy project.

* ``--spec-files={<spec-file>}``

   Filenames of the units of the input project to bind, separated by
   commas.

* ``-X=<name>=<value>``

   Scenario variable to pass to the project file.
...
* ``--no-subprojects``
   ...
   Only process units of the root project.

* ``--process-runtime``

   Also process the sources of the Ada runtime.

* ``--with-runtime=<path>``

   Location of the gnatpolyglot runtime to use. Defaults to <outputPath>/runtimes.

* ``--header-file=<file>``

   Prepend the content of the given file to all sources generated by GNATpolyglot.

If no unit is explicitly passed through ``--spec-files``, generate bindings
for all sources of the project tree, including those of the subprojects,
but not those of the Ada runtime.

If ``--process-runtime`` is set, also generate bindings for all the sources
of the runtime.

If ``--no-subprojects`` is set, only generate bindings for the root project.
Note that ``--process-runtime`` and ``--no-subprojects`` are illegal as soon as
``--spec-files`` is used.

Moreover, please note that GNATpolyglot will always make sure that the
generated library is usable, which means it may have to include entities
that were not part of the original files.
...
As a limitation, GNATpolyglot does not yet support discriminant components.
...
.. code:: ada
   ...
   type Foo_Shadow is new Foo with record
      SD: GNATpolyglot.Ada.Shadow_Data;
   end record;
...
Only one level of indirection is implemented; GNATpolyglot does not support
