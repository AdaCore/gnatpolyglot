...
How the bindings reach the Ada library
--------------------------------------

The generated Java interface talks to the bound Ada library through the
...
pointer to the internal Ada object and releases it automatically (through a
...
you override one of its methods, a dispatching call made inside the bound Ada
...
.. list-table::
   ...
   * - .. code:: ada
     ...
     - .. code:: java
          ...
          // Describe runs in the Ada library, but its dispatching call
...
Ada exceptions are bound as Java classes extending the runtime type
``com.adacore.gnatpolyglot.runtime.ada2java.AdaException``. When a bound
function raises, the exception is caught at the C ABI boundary, identified, and
re-thrown on the Java side as the matching class. The predefined Ada exceptions
(``Constraint_Error``, ``Program_Error``, ``Storage_Error`` and
``Tasking_Error``) are bound as well.
...
.. code:: sh
   ...
   # Build the JNI bridge (link against the Ada proxy library)
