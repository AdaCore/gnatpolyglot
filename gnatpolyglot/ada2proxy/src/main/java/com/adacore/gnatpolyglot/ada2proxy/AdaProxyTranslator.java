...
public class AdaProxyTranslator {
    ...
    private static class Visitor implements AdaProxyVisitor<ProxyObject> {
        ...
        public Proxy visit(AdaProxy proxy) {
            ...
            modules.add(
                    new Module(new FullyQualifiedName(Name.fromLower("gnatpolyglot")), List.of()));
            modules.add(
                    new Module(
                            new FullyQualifiedName(
                                    Name.fromLower("gnatpolyglot"), Name.fromLower("ada")),
                            List.of()));
            modules.add(
                    new Module(
                            new FullyQualifiedName(
                                    ...
                                    .flatMap(
                                            c ->
                                                    Array.memberFunctions(c).stream()
                                                            .map(f -> (Declaration) f))
                                    .toList()));
            return new Proxy(proxy.name, "ada", modules);
        }
