    ...
    public void generate(Path outputPath, Path runtimeLocation) throws IOException {
        ...
        Optional<Module> arrayModule =
                proxy.modules.stream()
                        .filter(
                                m ->
                                        m.name.names.get(0).equals(Name.fromLower("gnatpolyglot"))
                                                && m.name.getLastName()
        ...
        Path headerFilename =
                outputPath.resolve("gnatpolyglot_exceptions_" + proxy.name.toLower() + ".h");
        try (FileOutput packageSpec = new FileOutput(headerFilename)) {
            templateEngine.render(
        ...
        Path sourceFilename =
                outputPath.resolve("gnatpolyglot_exceptions_" + proxy.name.toLower() + ".cpp");
        try (FileOutput packageSpec = new FileOutput(sourceFilename)) {
            templateEngine.render(
                    "exception_handler.jte",
