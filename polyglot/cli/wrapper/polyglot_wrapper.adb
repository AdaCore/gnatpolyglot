...
procedure Polyglot_Wrapper is
   ...
   --  (so if you are running "prefix"/bin/polyglot, you would get "prefix").
   ...
   --  Locate Exec in <prefix>/libexec/polyglot/. The function
   --  allocates memory that needs to be freed by the caller.

   procedure Setup_Search_Paths;
   --  Set the POLYGLOT_RUNTIME environment variable to the installation's
   ...
   --  $PREFIX/lib/polyglot, to allow the worker processe that relies on
   ...
   function Executable_Location return String is
      ...
      --  e.g. "c:\usr\bin\polyglot.exe" or "..\bin\polyglot". Returns the absolute or
   ...
   function Locate_Exec_In_Libexec (Exec : String) return String_Access is
      ...
      Result  : constant String :=
        Compose
          (Compose (Compose (Executable_Location, "libexec"), "polyglot"),
           Exec);
   ...
   procedure Setup_Search_Paths is
      ...
      Lib_Polyglot     : constant String :=
        Compose (Compose (Prefix, "lib"), "polyglot");
      Share_Polyglot   : constant String :=
        Compose (Compose (Prefix, "share"), "polyglot");
   ...
   begin
      ...
      if not Environment_Variables.Exists ("POLYGLOT_RUNTIME") then
         Environment_Variables.Set ("POLYGLOT_RUNTIME", Runtime_Location);
      end if;
   end Setup_Search_Paths;

begin
   -- Locate the polyglot executable in libexec
   S := Locate_Exec_In_Libexec ("polyglot");

   if S = null then
      Put_Line ("error: polyglot not found");
