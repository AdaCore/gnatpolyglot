...
public:
    ...
    void foo(polyglot_ptr<polyglot_array<::test::Rec>> &a) const override {
       polyglot_ptr<polyglot_array<::test::Rec>> res{
           new polyglot_array<::test::Rec>{test::inc_arr(*a)}};
       a = res;
    }
...
int main() {
    ...
    test::Rec r2(1);
    test::Rec r3(2);
    polyglot_array<test::Rec> arr(1, 3);
    arr.set(1, r1);
    arr.set(2, r2);
    arr.set(3, r3);
    for (int i = 1; i <= 3; i++) {
    ...
    polyglot_ptr<polyglot_array<::test::Rec>> ptr{arr};
    Child c{};
    test::call_foo(c, ptr);
